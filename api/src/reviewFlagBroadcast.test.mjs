// Issue #660: a no-op re-save that clears a pending review flag (rows.ts's
// "allMatch" short-circuit) must broadcast the fresh row to other open tabs,
// same as the reorder-only fast path already does a few lines below it. Before
// this fix, the clear branch updated D1 and returned 200 to the caller's own
// tab, but every OTHER tab on that chapter kept showing the stale review-flag
// chip until its next book-lint refetch or chapter reload.
//
// This fork's clear branch is tn-only (rows.ts gates the clear on
// `kind === "tn"`), so coverage here is tn — a tq/twl re-save never reaches
// the clear branch and is not part of this fix.
//
// Run from api/ (node:sqlite is built in on the Volta-pinned Node 24; older
// Nodes need --experimental-sqlite):
//   node --experimental-strip-types --no-warnings src/reviewFlagBroadcast.test.mjs
//
...
console.log("\n[a no-op re-save that clears a review flag broadcasts the fresh row]");
{
  ...
  eq(roomName, `default:${BOOK}:36`, "broadcast targets the row's own workspace-scoped chapter room");
  ...
  eq(event.row.review_kind, null, "broadcasts the cleared flag — client-side refetch is covered in Shell");
...
console.log("\n[a reorder-only patch on a flagged row does NOT acknowledge the flag]");
{
  ...
  // broadcasts (the pre-existing reorder fast path), but with the flag intact.
  const { sqlite, patch, broadcasts } = freshApp();
  seedRow(sqlite);
